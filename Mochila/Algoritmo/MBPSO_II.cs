...
{
    public class MBPSO_II : Algoritmo
    {
        private Particula[] swarm { set; get; }

        private int numIteracionesBL { set; get; }
        ...
        private Algoritmo algoritmoLocal { set; get; }

        public MBPSO_II(Random r) { this.r = r; }

        //Inicialización de parametros
        public void inicializar(int swarmSize, int dimensiones, FuncionObjetivo fx, int NI, double peso, int opcionArreglo, double factorMejorLocal, double factorMejorGlobal, int velocidadMaxima, int numIteracionesBL)
        {
            pesoMaximo = peso;
            numIteraciones = NI;
            Best = null;
            this.numIteracionesBL = numIteracionesBL;
            ...
            algoritmoLocal = new HillClimbing(this.numIteracionesBL, fx, r, pesoMaximo);
        ...
        private void actualizarBest()
        {
            ...
            for (; i < swarm.Length; i++)
            {
                if (swarm[i].Best == null || fx.getCalidad(swarm[i]) > fx.getCalidad(swarm[i].Best))
                    swarm[i].Best = ejecutarBusquedaLocal(swarm[i].copy());
                if (Best == null || fx.getCalidad(swarm[i].Best) > fx.getCalidad(Best))
                    Best = ejecutarBusquedaLocal(swarm[i].Best.copy());
            }
        }
